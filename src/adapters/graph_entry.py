...
Propósito:
  - Permitir correr una invocación del grafo en modo "static" (router determinístico)
    o "llm" (ToolNode + tools_condition) usando argumento CLI o variable de entorno.
...
Variables de entorno:
  - SELECTOR_MODE: "static" | "llm" (se usa si no pasas el modo por CLI; default "static").
  - OPENAI_API_KEY: requerida solo en modo "llm". Se carga desde `.env` vía load_settings().
"""
...
def main() -> None:
    ...
    Flujo:
      ...
      6) Reconstruye `State` y muestra bloque final, artifacts, errores y confianza.
    """
    ...
    last = final_state.working_memory[-1]["content"] if final_state.working_memory else {}
